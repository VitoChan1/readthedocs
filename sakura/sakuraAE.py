...
class sakuraAE(object):
    ...
    def generate_splits(self):
        ...
        self.splits['all'] = np.ones(len(self.count_data), dtype=np.bool_)
        ...
        if self.config['overall_train_test_split']['type'] == 'auto':
            ...
            self.splits['overall_train'] = overall_train_test_split['train'].astype(np.bool_)
            self.splits['overall_test'] = overall_train_test_split['test'].astype(np.bool_)
        ...
        if len(self.selected_pheno) > 0:
            for cur_pheno in self.selected_pheno:
                ...
                if self.count_data.pheno_meta[cur_pheno]['split']['type'] == 'auto':
                    ...
                    self.splits[train_split_id] = cur_label_train_test_split['train'].astype(np.bool_)
                    self.splits[test_split_id] = cur_label_train_test_split['test'].astype(np.bool_)
        ...
        if len(self.selected_signature) > 0:
            for cur_signature in self.selected_signature:
                ...
                if self.signature_config[cur_signature]['split']['type'] == 'auto':
                    ...
                    self.splits[train_split_id] = cur_signature_train_test_split['train'].astype(np.bool_)
                    self.splits[test_split_id] = cur_signature_train_test_split['test'].astype(np.bool_)
